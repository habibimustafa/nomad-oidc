...
get_oidc_token() {
  ...
  BINARY_ENCRYPTED_NOMAD_TOKEN=$(echo -n "${nomad_token}" | openssl enc -aes-256-cbc -pbkdf2 -salt -k "${ENCRYPTION_PASSWORD}");
  ENCRYPTED_NOMAD_TOKEN=$(echo -n "$BINARY_ENCRYPTED_NOMAD_TOKEN" | base64);
